...
template <class Key> class LinkedHashSet {
public:

   class MapElement {
   public:

      MapElement(Key* key, MapElement* next) {
         this->key = key;
         this->prev = NULL;
         this->next = next;
      }
   ...
   void Add(Key* key) {
      Remove(key);
      MapElement* e = new MapElement(key, root);
      if (root != NULL) {
         root->prev = e;
      }
      root = e;
   ...
   void AddAll(std::list<Key*>& list) {
      for (typename std::list<Key*>::const_iterator iter = list.begin(), end = list.end(); iter != end; ++iter) {
         Add(*iter);
      }
   }

   void AddAll(LinkedHashSet* set) {
      for (MapElement* e = set->root; e != NULL; e = e->next) {
         Add(e->key);
      }
   }

   void RemoveAll(std::list<Key*>& list) {
      for (typename std::list<Key*>::const_iterator iter = list.begin(), end = list.end(); iter != end; ++iter) {
         Remove(*iter);
      }
   }

   void RemoveAll(LinkedHashSet* set) {
      for (MapElement* e = set->root; e != NULL; e = e->next) {
         Remove(e->key);
      }
   }
